...
pub const Tetris = struct {
    ...
    pub fn draw(t: *Tetris, comptime g: type) void {
                    ...
                    switch (cell) {
                        Cell.Color => |color| {
                            ...
                            fillRect(
                                ...
                                g,
                                color,
                                @as(f32, @floatFromInt(cell_left)),
                                @as(f32, @floatFromInt(cell_top)),
                                cell_size,
                                cell_size,
                            );
                        },
        ...
        }
        {
            ...
            const len = @as(usize, @intCast(c.sprintf(&score_text_buf, "%d", t.score)));
        ...
        }
        {
            ...
            const len = @as(usize, @intCast(c.sprintf(&text_buf, "%d", t.level)));
    ...
    fn drawPieceWithColor(t: *Tetris, comptime g: type, piece: Piece, left: i32, top: i32, rot: usize, color: Vec4) void {
        for (piece.layout[rot], 0..) |row, y| {
            for (row, 0..) |is_filled, x| {
                ...
                const abs_x = @as(f32, @floatFromInt(left + @as(i32, @intCast(x)) * cell_size));
                const abs_y = @as(f32, @floatFromInt(top + @as(i32, @intCast(y)) * cell_size));
    ...
    pub fn nextFrame(t: *Tetris, elapsed: f64) void {
        ...
        if (t.screen_shake_elapsed < t.screen_shake_timeout) {
            ...
            if (t.screen_shake_elapsed >= t.screen_shake_timeout) {
                ...
                const offset = @as(f32, @floatCast(amplitude * -c.sin(2.0 * PI * t.screen_shake_elapsed * rate)));
                t.projection = Mat4x4.ortho(
                    0.0,
                    @as(f32, @floatFromInt(t.framebuffer_width)),
    ...
    pub fn userRotateCurPiece(t: *Tetris, rot: i8) void {
        if (t.game_over or t.is_paused) return;
        const new_rot = @as(usize, @intCast(@rem(@as(isize, @intCast(t.cur_piece_rot)) + rot + 4, 4)));
    ...
    fn lockPiece(t: *Tetris) void {
        ...
        for (t.cur_piece.layout[t.cur_piece_rot], 0..) |row, y| {
            for (row, 0..) |is_filled, x| {
                ...
                if (abs_x >= 0 and abs_y >= 0 and abs_x < grid_width and abs_y < grid_height) {
                    t.grid[@as(usize, @intCast(abs_y))][@as(usize, @intCast(abs_x))] = Cell{ .Color = t.cur_piece.color };
                }
            }
        }
        ...
        while (y >= 0) {
            ...
            for (t.grid[@as(usize, @intCast(y))]) |cell| {
                const filled = switch (cell) {
                    Cell.Empty => false,
                    else => true,
                };
            ...
            if (all_filled) {
                rows_deleted += 1;
                deleteRow(t, @as(usize, @intCast(y)));
            } else {
                y -= 1;
            }
        }
    ...
    pub fn resetProjection(t: *Tetris) void {
        t.projection = Mat4x4.ortho(
            0.0,
            @as(f32, @floatFromInt(t.framebuffer_width)),
            @as(f32, @floatFromInt(t.framebuffer_height)),
            0.0,
        );
    }
    ...
    fn cellEmpty(t: *Tetris, x: i32, y: i32) bool {
        return switch (t.grid[@as(usize, @intCast(y))][@as(usize, @intCast(x))]) {
            Cell.Empty => true,
            else => false,
        };
    }
    ...
    fn doGameOver(t: *Tetris) void {
        ...
        for (t.grid, 0..) |row, y| {
            for (row, 0..) |cell, x| {
                ...
                const left = @as(f32, @floatFromInt(board_left + x * cell_size));
                const top = @as(f32, @floatFromInt(board_top + y * cell_size));
...
fn randIntRangeLessThan(comptime T: type, at_least: T, less_than: T) T {
    return @as(T, @truncate(at_least + @mod(@as(T, @intCast(c.rand())), (less_than - at_least))));
}
...
fn randInt(comptime T: type) T {
    ...
    for (rand_bytes[0..]) |*byte| {
        byte.* = @as(u8, @truncate(@as(c_uint, @bitCast(c.rand()))));
    }
    return @as(T, @bitCast(rand_bytes));
}
