...
pub const PngImage = struct {
    ...
    pub fn create(compressed_bytes: []const u8) !PngImage {
        ...
        if (c.stbi_info_from_memory(compressed_bytes.ptr, @as(c_int, @intCast(compressed_bytes.len)), &width, &height, null) == 0) {
            return error.NotPngFile;
        }

        if (width <= 0 or height <= 0) return error.NoPixels;
        pi.width = @as(u32, @intCast(width));
        pi.height = @as(u32, @intCast(height));

        // Not validating channel_count because it gets auto-converted to 4

        if (c.stbi_is_16_bit_from_memory(compressed_bytes.ptr, @as(c_int, @intCast(compressed_bytes.len))) != 0) {
        ...
        const image_data = c.stbi_load_from_memory(compressed_bytes.ptr, @as(c_int, @intCast(compressed_bytes.len)), &width, &height, null, channel_count);
