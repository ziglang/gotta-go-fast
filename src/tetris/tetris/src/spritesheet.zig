...
pub const Spritesheet = struct {
    ...
    pub fn draw(s: *Spritesheet, as: AllShaders, index: usize, mvp: Mat4x4) void {
        ...
        c.glEnableVertexAttribArray(@as(c.GLuint, @intCast(as.texture_attrib_position)));
        c.glVertexAttribPointer(@as(c.GLuint, @intCast(as.texture_attrib_position)), 3, c.GL_FLOAT, c.GL_FALSE, 0, null);

        c.glBindBuffer(c.GL_ARRAY_BUFFER, s.tex_coord_buffers[index]);
        c.glEnableVertexAttribArray(@as(c.GLuint, @intCast(as.texture_attrib_tex_coord)));
        c.glVertexAttribPointer(@as(c.GLuint, @intCast(as.texture_attrib_tex_coord)), 2, c.GL_FLOAT, c.GL_FALSE, 0, null);
    ...
    pub fn init(s: *Spritesheet, compressed_bytes: []const u8, w: usize, h: usize) !void {
        ...
        c.glTexImage2D(
            ...
            0,
            c.GL_RGBA,
            @as(c_int, @intCast(s.img.width)),
            @as(c_int, @intCast(s.img.height)),
            0,
            c.GL_RGBA,
            c.GL_UNSIGNED_BYTE,
            @as(*anyopaque, @ptrCast(&s.img.raw[0])),
        );
        ...
        const vertexes = [_][3]c.GLfloat{
            ...
            [_]c.GLfloat{ 0.0, @as(c.GLfloat, @floatFromInt(h)), 0.0 },
            [_]c.GLfloat{ @as(c.GLfloat, @floatFromInt(w)), 0.0, 0.0 },
            [_]c.GLfloat{ @as(c.GLfloat, @floatFromInt(w)), @as(c.GLfloat, @floatFromInt(h)), 0.0 },
        };

        c.glBindBuffer(c.GL_ARRAY_BUFFER, s.vertex_buffer);
        c.glBufferData(c.GL_ARRAY_BUFFER, 4 * 3 * @sizeOf(c.GLfloat), @as(*const anyopaque, @ptrCast(&vertexes[0][0])), c.GL_STATIC_DRAW);
        ...
        c.glGenBuffers(@as(c.GLint, @intCast(s.tex_coord_buffers.len)), s.tex_coord_buffers.ptr);
        errdefer c.glDeleteBuffers(@as(c.GLint, @intCast(s.tex_coord_buffers.len)), &s.tex_coord_buffers[0]);
        ...
        for (s.tex_coord_buffers, 0..) |tex_coord_buffer, i| {
            ...
            const x = @as(f32, @floatFromInt(col * w));
            const y = @as(f32, @floatFromInt(row * h));

            const img_w = @as(f32, @floatFromInt(s.img.width));
            const img_h = @as(f32, @floatFromInt(s.img.height));
            ...
            c.glBufferData(c.GL_ARRAY_BUFFER, 4 * 2 * @sizeOf(c.GLfloat), @as(*const anyopaque, @ptrCast(&tex_coords[0][0])), c.GL_STATIC_DRAW);
        }
    }

    pub fn deinit(s: *Spritesheet) void {
        c.glDeleteBuffers(@as(c.GLint, @intCast(s.tex_coord_buffers.len)), s.tex_coord_buffers.ptr);
