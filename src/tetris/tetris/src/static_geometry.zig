...
pub const StaticGeometry = struct {
    ...
    pub fn create() StaticGeometry {
        ...
        c.glBufferData(c.GL_ARRAY_BUFFER, 4 * 3 * @sizeOf(c.GLfloat), @as(*const anyopaque, @ptrCast(&rect_2d_vertexes[0][0])), c.GL_STATIC_DRAW);
        ...
        c.glBufferData(c.GL_ARRAY_BUFFER, 4 * 2 * @sizeOf(c.GLfloat), @as(*const anyopaque, @ptrCast(&rect_2d_tex_coords[0][0])), c.GL_STATIC_DRAW);
        ...
        c.glBufferData(c.GL_ARRAY_BUFFER, 3 * 3 * @sizeOf(c.GLfloat), @as(*const anyopaque, @ptrCast(&triangle_2d_vertexes[0][0])), c.GL_STATIC_DRAW);
        ...
        c.glBufferData(c.GL_ARRAY_BUFFER, 3 * 2 * @sizeOf(c.GLfloat), @as(*const anyopaque, @ptrCast(&triangle_2d_tex_coords[0][0])), c.GL_STATIC_DRAW);
