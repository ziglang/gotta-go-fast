...
pub const ShaderProgram = struct {
    ...
    pub fn create(
        ...
        const message = c.malloc(@as(c_ulong, @intCast(error_size))) orelse return error.OutOfMemory;
        c.glGetProgramInfoLog(sp.program_id, error_size, &error_size, @as([*:0]u8, @ptrCast(message)));
...
fn initGlShader(source: []const u8, name: [*:0]const u8, kind: c.GLenum) !c.GLuint {
    ...
    const message = c.malloc(@as(c_ulong, @intCast(error_size))) orelse return error.OutOfMemory;
    c.glGetShaderInfoLog(shader_id, error_size, &error_size, @as([*:0]u8, @ptrCast(message)));
