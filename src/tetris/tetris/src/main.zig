...
fn keyCallback(
    ...
    _ = mods;
    _ = scancode;
    const t = @as(*Tetris, @ptrCast(@alignCast(c.glfwGetWindowUserPointer(win).?)));
...
pub fn main2() !void {
    ...
    c.srand(@as(c_uint, @truncate(@as(c_ulong, @bitCast(c.time(null))))));
    ...
    c.glfwSetWindowUserPointer(window, @as(*anyopaque, @ptrCast(t)));
...
pub fn fillRectMvp(color: Vec4, mvp: Mat4x4) void {
    ...
    c.glEnableVertexAttribArray(@as(c.GLuint, @intCast(all_shaders.primitive_attrib_position)));
    c.glVertexAttribPointer(@as(c.GLuint, @intCast(all_shaders.primitive_attrib_position)), 3, c.GL_FLOAT, c.GL_FALSE, 0, null);
...
pub fn drawText(t: *Tetris, text: []const u8, left: i32, top: i32, size: f32) void {
    for (text, 0..) |col, i| {
        if (col <= '~') {
            ...
            const model = Mat4x4.identity.translate(char_left, @as(f32, @floatFromInt(top)), 0.0).scale(size, size, 0.0);
            const mvp = t.projection.mult(model);
