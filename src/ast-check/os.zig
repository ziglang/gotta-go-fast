...
pub fn isCygwinPty(handle: fd_t) bool {
    ...
    if (windows.kernel32.GetFileInformationByHandleEx(
        ...
        @as(*anyopaque, @ptrCast(&name_info_bytes)),
        name_info_bytes.len,
    ) == 0) {
        return false;
    }
...
pub fn waitpid(pid: pid_t, flags: u32) WaitPidResult {
    ...
    while (true) {
        const rc = system.waitpid(pid, &status, if (builtin.link_libc) @as(c_int, @intCast(flags)) else flags);
        switch (errno(rc)) {
            .SUCCESS => return .{
...
pub fn faccessatW(dirfd: fd_t, sub_path_w: [*:0]const u16, mode: u32, flags: u32) AccessError!void {
    ...
    var nt_name = windows.UNICODE_STRING{
        ...
        .Buffer = @as([*]u16, @ptrFromInt(@intFromPtr(sub_path_w))),
    };
    ...
    if (builtin.link_libc) {
        switch (system.dl_iterate_phdr(struct {
            fn callbackC(info: *dl_phdr_info, size: usize, data: ?*anyopaque) callconv(.C) c_int {
                const context_ptr = @as(*const Context, @ptrCast(@alignCast(data)));
...
pub fn sendmsg(
    ...
    while (true) {
        const rc = system.sendmsg(sockfd, @as(*const std.x.os.Socket.Message, @ptrCast(&msg)), @as(c_int, @intCast(flags)));
        if (builtin.os.tag == .windows) {
            if (rc == windows.ws2_32.SOCKET_ERROR) {
                switch (windows.ws2_32.WSAGetLastError()) {
                    .WSAEACCES => return error.AccessDenied,
