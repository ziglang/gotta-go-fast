...
pub fn resolveInst(sema: *Sema, zir_ref: Zir.Inst.Ref) Air.Inst.Ref {
    ...
    return sema.inst_map.get(@as(u32, @intCast(i))).?;
}
...
fn resolveAlreadyCoercedInt(
    ...
    switch (@typeInfo(Int).Int.signedness) {
        .signed => return @as(Int, @intCast(val.toSignedInt())),
        .unsigned => return @as(Int, @intCast(val.toUnsignedInt())),
    }
}
...
fn resolveAlign(
    ...
    const alignment = @as(u16, @intCast(alignment_big)); // We coerce to u16 in the prev line.
...
fn zirErrorSetDecl(
    ...
    error_set.* = .{
        ...
        .names_ptr = names.ptr,
        .names_len = @as(u32, @intCast(names.len)),
    };
    try new_decl.finalizeNewArena(&new_decl_arena);
    return sema.analyzeDeclVal(block, src, new_decl);
}
...
fn zirRetPtr(
    ...
    const src: LazySrcLoc = .{ .node_offset = @as(i32, @bitCast(extended.operand)) };
...
fn zirLoop(sema: *Sema, parent_block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    sema.air_instructions.items(.data)[loop_inst].ty_pl.payload = sema.addExtraAssumeCapacity(
        Air.Block{ .body_len = @as(u32, @intCast(loop_block.instructions.items.len)) },
    );
...
fn analyzeBlockBody(
    ...
    for (merges.br_list.items) |br| {
        ...
        try sema.air_instructions.ensureUnusedCapacity(gpa, 2);
        const sub_block_inst = @as(Air.Inst.Index, @intCast(sema.air_instructions.len));
        ...
        sema.air_instructions.appendAssumeCapacity(.{
            ...
            .data = .{ .ty_pl = .{
                ...
                .payload = sema.addExtraAssumeCapacity(Air.Block{
                    .body_len = @as(u32, @intCast(coerce_block.instructions.items.len)),
                }),
            } },
        });
...
fn zirCall(
    ...
    const modifier = @as(std.builtin.CallOptions.Modifier, @enumFromInt(extra.data.flags.packed_modifier));
...
fn analyzeCall(
    ...
    const result: Air.Inst.Ref = if (is_inline_call) res: {
            ...
            if (is_comptime_call) {
                if (mod.memoized_calls.get(memoized_call_key)) |result| {
                    ...
                    sema.air_instructions.set(block_inst, .{
                        ...
                        .data = .{ .ty_pl = .{
                            .ty = ty_inst,
                            .payload = @as(u32, @intCast(sema.air_values.items.len - 1)),
                        } },
                    });
        ...
        std.hash.autoHash(&hasher, @intFromPtr(module_fn));

        const comptime_tvs = try sema.arena.alloc(TypedValue, func_ty_info.param_types.len);

        for (func_ty_info.param_types, 0..) |param_ty, i| {
        ...
        gop.key_ptr.* = new_module_func;
        {
            ...
            try child_sema.inst_map.ensureUnusedCapacity(gpa, @as(u32, @intCast(uncasted_args.len)));
            var arg_i: usize = 0;
...
fn zirIntToError(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    if (try sema.resolveDefinedValue(block, operand_src, op)) |value| {
        ...
        payload.* = .{
            .base = .{ .tag = .@"error" },
            .data = .{ .name = sema.mod.error_name_list.items[@as(usize, @intCast(int))] },
        };
...
fn zirMergeErrorSets(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    new_error_set.* = .{
        ...
        .names_len = @as(u32, @intCast(new_names.len)),
    };
...
fn zirSwitchBlock(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    while (multi_i < multi_cases_len) : (multi_i += 1) {
        ...
        if (ranges_len == 0) {
            ...
            var range_i: usize = 0;
            while (range_i < ranges_len) : (range_i += 1) {
                const first_ref = @as(Zir.Inst.Ref, @enumFromInt(sema.code.extra[extra_index]));
                extra_index += 1;
                const last_ref = @as(Zir.Inst.Ref, @enumFromInt(sema.code.extra[extra_index]));
                extra_index += 1;
...
fn zirArrayCat(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    if (try sema.resolveDefinedValue(block, lhs_src, lhs)) |lhs_val| {
        if (try sema.resolveDefinedValue(block, rhs_src, rhs)) |rhs_val| {
            ...
            const final_len_including_sent = final_len + @intFromBool(res_sent != null);
...
fn zirArrayMul(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    const final_len_including_sent = final_len + @intFromBool(mulinfo.sentinel != null);
    ...
    switch (ty.zigTypeTag()) {
        .Type => return sema.addConstant(
            ...
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Type)),
        ...
        .Void => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Void)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .Bool => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Bool)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .NoReturn => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.NoReturn)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .ComptimeFloat => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.ComptimeFloat)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .ComptimeInt => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.ComptimeInt)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .Undefined => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Undefined)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .Null => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Null)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .EnumLiteral => return sema.addConstant(
            type_info_ty,
            try Value.Tag.@"union".create(sema.arena, .{
                .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.EnumLiteral)),
                .val = Value.initTag(.unreachable_value),
            }),
        ),
        .Fn => {
            ...
            field_values[0] = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(info.cc));
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Fn)),
        ...
        .Int => {
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Int)),
                    .val = try Value.Tag.@"struct".create(sema.arena, field_values),
                }),
            );
        },
        .Float => {
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Float)),
                    .val = try Value.Tag.@"struct".create(sema.arena, field_values),
                }),
            );
        },
        .Pointer => {
            ...
            // size: Size,
            field_values[0] = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(info.size));
            // is_const: bool,
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Pointer)),
                    .val = try Value.Tag.@"struct".create(sema.arena, field_values),
                }),
            );
        },
        .Array => {
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Array)),
                    .val = try Value.Tag.@"struct".create(sema.arena, field_values),
                }),
            );
        },
        .Optional => {
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.Optional)),
                    .val = try Value.Tag.@"struct".create(sema.arena, field_values),
                }),
            );
        },
        .ErrorUnion => {
            ...
            return sema.addConstant(
                type_info_ty,
                try Value.Tag.@"union".create(sema.arena, .{
                    .tag = try Value.Tag.enum_field_index.create(sema.arena, @intFromEnum(std.builtin.TypeId.ErrorUnion)),
                    .val = try Value.Tag.@"struct".create(sema.arena, field_values),
                }),
            );
        },
...
fn zirStructInit(sema: *Sema, block: *Block, inst: Zir.Inst.Index, is_ref: bool) CompileError!Air.Inst.Ref {
    ...
    if (resolved_ty.castTag(.@"struct")) |struct_payload| {
        ...
        for (found_fields, 0..) |field_type_inst, i| {
...
fn zirArrayInit(
    ...
    for (args, 0..) |arg, i| resolved_args[i] = sema.resolveInst(arg);
...
fn zirBoolToInt(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    if (try sema.resolveMaybeUndefVal(block, operand_src, operand)) |val| {
        ...
        return bool_ints[@intFromBool(val.toBool())];
    }
...
fn zirReify(sema: *Sema, block: *Block, inst: Zir.Inst.Index) CompileError!Air.Inst.Ref {
    ...
    switch (@as(std.builtin.TypeId, @enumFromInt(tag_index))) {
        ...
        .Int => {
            ...
            const bits = @as(u16, @intCast(bits_val.toUnsignedInt()));
...
fn zirFuncExtended(
    ...
    const small = @as(Zir.Inst.ExtendedFunc.Small, @bitCast(extended.small));
            ...
            switch (array_ty.ptrSize()) {
                .Slice => {
                    ...
                    const runtime_src = if (maybe_slice_val) |slice_val| rs: {
                        ...
                        const index = @as(usize, @intCast(index_val.toUnsignedInt()));
...
fn elemPtrArray(
    ...
    if (try sema.resolveDefinedValue(block, src, array_ptr)) |array_ptr_val| {
        if (try sema.resolveDefinedValue(block, elem_index_src, elem_index)) |index_val| {
            ...
            const elem_ptr = try array_ptr_val.elemPtr(sema.arena, @as(usize, @intCast(index_u64)));
            return sema.addConstant(result_ty, elem_ptr);
        }
    }
    ...
    switch (dest_ty.zigTypeTag()) {
        ...
        .Enum => switch (inst_ty.zigTypeTag()) {
            .EnumLiteral => {
                ...
                return sema.addConstant(
                    dest_ty,
                    try Value.Tag.enum_field_index.create(arena, @as(u32, @intCast(field_index))),
                );
            },
            .Union => blk: {
...
fn beginComptimePtrMutation(
    ...
    switch (ptr_val.tag()) {
        ...
        .field_ptr => {
            ...
            const field_index = @as(u32, @intCast(field_ptr.field_index));
...
fn analyzeDeclRef(sema: *Sema, decl: *Decl) CompileError!Air.Inst.Ref {
    ...
    if (decl_tv.val.castTag(.variable)) |payload| {
        ...
        const alignment: u32 = if (decl.align_val.tag() == .null_value)
            ...
            @as(u32, @intCast(decl.align_val.toUnsignedInt()));
        const ty = try Type.ptr(sema.arena, .{
...
fn cmpNumeric(
    ...
    if (try sema.resolveMaybeUndefVal(block, lhs_src, lhs)) |lhs_val| {
        ...
        lhs_bits += @intFromBool(is_unsigned and dest_int_is_signed);
    ...
    if (try sema.resolveMaybeUndefVal(block, rhs_src, rhs)) |rhs_val| {
        ...
        rhs_bits += @intFromBool(is_unsigned and dest_int_is_signed);
...
fn semaStructFields(
    ...
    while (field_i < fields_len) : (field_i += 1) {
        ...
        if (has_default) {
            const default_ref = @as(Zir.Inst.Ref, @enumFromInt(zir.extra[extra_index]));
            extra_index += 1;
...
fn appendRefsAssumeCapacity(sema: *Sema, refs: []const Air.Inst.Ref) void {
    const coerced = @as([]const u32, @bitCast(refs));
