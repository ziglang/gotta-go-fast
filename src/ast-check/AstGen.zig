...
fn arrayInitExprRlNone(
    ...
    const init_inst = try gz.addPlNode(tag, node, Zir.Inst.MultiOp{
        .operands_len = @as(u32, @intCast(elem_list.len)),
    });
...
fn arrayInitExprRlPtrInner(
    ...
    _ = try gz.addPlNode(.validate_array_init, node, Zir.Inst.Block{
        .body_len = @as(u32, @intCast(elem_ptr_list.len)),
    });
...
fn structInitExprRlNone(
    ...
    const init_inst = try gz.addPlNode(tag, node, Zir.Inst.StructInitAnon{
        .fields_len = @as(u32, @intCast(fields_list.len)),
    });
...
fn structInitExprRlPtrInner(
    ...
    _ = try gz.addPlNode(.validate_struct_init, node, Zir.Inst.Block{
        .body_len = @as(u32, @intCast(field_ptr_list.len)),
    });
            ...
            .gen_zir => {
                ...
                if (block_gz.break_result_loc == .block_ptr) {
                    ...
                    if (have_store_to_block) {
                        ...
                        const store_inst = @as(u32, @intCast(zir_tags.len - 2));
...
fn emitDbgNode(gz: *GenZir, node: Ast.Node.Index) !void {
    ...
    const line = @as(u32, @intCast(astgen.source_line));
    const column = @as(u32, @intCast(astgen.source_column));
...
fn fnDecl(
    ...
    const func_inst: Zir.Inst.Ref = if (body_node == 0) func: {
        ...
        const lbrace_line = @as(u32, @intCast(astgen.source_line));
        const lbrace_column = @as(u32, @intCast(astgen.source_column));
    ...
    if (align_inst != .none) {
        wip_decls.payload.appendAssumeCapacity(@intFromEnum(align_inst));
    }

    if (has_section_or_addrspace) {
        wip_decls.payload.appendAssumeCapacity(@intFromEnum(section_inst));
        wip_decls.payload.appendAssumeCapacity(@intFromEnum(addrspace_inst));
    }
}
...
fn structDeclInner(
    ...
    for (container_decl.ast.members) |member_node| {
        ...
        fields_data.appendAssumeCapacity(@intFromEnum(field_type));
        ...
        cur_bit_bag = (cur_bit_bag >> bits_per_field) |
            (@as(u32, @intFromBool(have_align)) << 28) |
            (@as(u32, @intFromBool(have_value)) << 29) |
            (@as(u32, @intFromBool(is_comptime)) << 30) |
            (@as(u32, @intFromBool(unused)) << 31);

        if (have_align) {
            const align_inst = try expr(&block_scope, &namespace.base, align_rl, member.ast.align_expr);
            fields_data.appendAssumeCapacity(@intFromEnum(align_inst));
        }
        if (have_value) {
            ...
            fields_data.appendAssumeCapacity(@intFromEnum(default_inst));
    ...
    }
    {
        ...
        if (empty_slot_count < fields_per_u32) {
            cur_bit_bag >>= @as(u5, @intCast(empty_slot_count * bits_per_field));
        }
    }
    {
        ...
        if (empty_slot_count < WipDecls.fields_per_u32) {
            wip_decls.cur_bit_bag >>= @as(u5, @intCast(empty_slot_count * WipDecls.bits_per_field));
        }
    }
    ...
    try gz.setStruct(decl_inst, .{
        .src_node = node,
        .layout = layout,
        .body_len = @as(u32, @intCast(block_scope.instructions.items.len)),
        .fields_len = @as(u32, @intCast(field_index)),
        .decls_len = @as(u32, @intCast(wip_decls.decl_index)),
...
fn unionDeclInner(
    ...
    for (members) |member_node| {
        ...
        const unused = false;
        cur_bit_bag = (cur_bit_bag >> bits_per_field) |
            (@as(u32, @intFromBool(have_type)) << 28) |
            (@as(u32, @intFromBool(have_align)) << 29) |
            (@as(u32, @intFromBool(have_value)) << 30) |
            (@as(u32, @intFromBool(unused)) << 31);
        ...
        if (have_value) {
            ...
            fields_data.appendAssumeCapacity(@intFromEnum(tag_value));
        }
        ...
        .keyword_enum => {
            ...
            for (container_decl.ast.members) |member_node| {
                ...
                cur_bit_bag = (cur_bit_bag >> 1) |
                    (@as(u32, @intFromBool(have_value)) << 31);
                ...
                if (have_value) {
                    ...
                    fields_data.appendAssumeCapacity(@intFromEnum(tag_value_inst));
                }
            ...
            }
            {
                ...
                if (empty_slot_count < 32) {
                    cur_bit_bag >>= @as(u5, @intCast(empty_slot_count));
                }
            }
            {
                const empty_slot_count = WipDecls.fields_per_u32 - (wip_decls.decl_index % WipDecls.fields_per_u32);
...
fn errorSetDecl(gz: *GenZir, rl: ResultLoc, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    const result = try gz.addPlNode(.error_set_decl, node, Zir.Inst.ErrorSetDecl{
        .fields_len = @as(u32, @intCast(field_names.items.len)),
    });
...
fn setCondBrPayload(
    ...
    zir_datas[condbr].pl_node.payload_index = astgen.addExtraAssumeCapacity(Zir.Inst.CondBr{
        .condition = cond,
        .then_body_len = @as(u32, @intCast(then_scope.instructions.items.len)),
        .else_body_len = @as(u32, @intCast(else_scope.instructions.items.len)),
    });
...
fn switchExpr(
    ...
    for (case_nodes) |case_node| {
        ...
        if (is_multi_case) {
            ...
            for (case.ast.values) |item_node| {
                ...
                try multi_cases_payload.append(gpa, @intFromEnum(item_inst));
            }
            ...
            multi_cases_payload.items[header_index + 2] = @as(u32, @intCast(case_scope.instructions.items.len));
            ...
            special_case_payload.appendAssumeCapacity(@as(u32, @intCast(case_scope.instructions.items.len)));
            ...
            try scalar_cases_payload.ensureUnusedCapacity(gpa, 2 + // item + body_len
                case_scope.instructions.items.len);
            scalar_cases_payload.appendAssumeCapacity(@intFromEnum(item_inst));
            scalar_cases_payload.appendAssumeCapacity(@as(u32, @intCast(case_scope.instructions.items.len)));
    ...
    switch (strat.tag) {
        ...
        .break_void => {
            ...
            extra_index += 2;
            extra_index += @intFromBool(multi_cases_len != 0);
...
fn builtinCall(
    ...
    switch (info.tag) {
        ...
        .compile_log => {
            ...
            for (params, 0..) |param, i| arg_refs[i] = try expr(gz, scope, .none, param);
    ...
    switch (rl) {
        ...
        .ty => |ty_inst| {
            ...
            switch ((@as(u64, @intFromEnum(ty_inst)) << 32) | @as(u64, @intFromEnum(result))) {
                ...
                as_ty | @intFromEnum(Zir.Inst.Ref.c_void_type),
...
const GenZir = struct {
    ...
    fn calcLine(gz: GenZir, node: Ast.Node.Index) u32 {
        ...
        return @as(u32, @intCast(gz.decl_line + astgen.source_line));
    }
    ...
    fn setBoolBrBody(gz: GenZir, inst: Zir.Inst.Index) !void {
        ...
        zir_datas[inst].bool_br.payload_index = gz.astgen.addExtraAssumeCapacity(
            Zir.Inst.Block{ .body_len = @as(u32, @intCast(gz.instructions.items.len)) },
        );
    ...
    fn setBlockBodyEliding(gz: GenZir, inst: Zir.Inst.Index) !void {
        ...
        const block_pl_index = gz.astgen.addExtraAssumeCapacity(Zir.Inst.Block{
            .body_len = @as(u32, @intCast(gz.instructions.items.len)),
        });
            ...
            args.is_extern)
        {
            try astgen.extra.ensureUnusedCapacity(
                ...
                @typeInfo(Zir.Inst.ExtendedFunc).Struct.fields.len +
                    ...
                    @intFromBool(args.cc != .none),
            );
            const payload_index = astgen.addExtraAssumeCapacity(Zir.Inst.ExtendedFunc{
                .src_node = gz.nodeIndexToRelative(args.src_node),
                .param_block = args.param_block,
                .ret_body_len = @as(u32, @intCast(args.ret_ty.len)),
                .body_len = @as(u32, @intCast(args.body.len)),
            });
            if (args.lib_name != 0) {
                astgen.extra.appendAssumeCapacity(args.lib_name);
            }
            if (args.cc != .none) {
                astgen.extra.appendAssumeCapacity(@intFromEnum(args.cc));
            }
            ...
            astgen.instructions.appendAssumeCapacity(.{
                ...
                .data = .{ .extended = .{
                    .opcode = .func,
                    .small = @as(u16, @bitCast(Zir.Inst.ExtendedFunc.Small{
    ...
    fn addCall(
        ...
        const payload_index = gz.astgen.addExtraAssumeCapacity(Call{
            .callee = callee,
            .flags = .{
                .packed_modifier = @as(Call.Flags.PackedModifier, @intCast(@intFromEnum(modifier))),
                .args_len = @as(Call.Flags.PackedArgsLen, @intCast(args.len)),
            },
        });
    ...
    fn addParam(
        ...
        const payload_index = gz.astgen.addExtraAssumeCapacity(Zir.Inst.Param{
            .name = name,
            .body_len = @as(u32, @intCast(body.len)),
        });
