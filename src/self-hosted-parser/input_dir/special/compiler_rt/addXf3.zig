...
fn normalize(comptime T: type, significand: *std.meta.Int(.unsigned, @typeInfo(T).Float.bits)) i32 {
    ...
    significand.* <<= @as(S, @intCast(shift));
    return 1 - shift;
}
...
fn addXf3(comptime T: type, a: T, b: T) T {
    ...
    const @"align" = @as(Z, @intCast(aExponent - bExponent));
    if (@"align" != 0) {
        if (@"align" < typeWidth) {
    ...
    if (subtraction) {
        ...
        if (aSignificand < implicitBit << 3) {
            const shift = @as(i32, @intCast(@clz(aSignificand))) - @as(i32, @intCast(@clz(implicitBit << 3)));
    ...
    if (aExponent <= 0) {
        ...
        const shift = @as(Z, @intCast(1 - aExponent));
        const sticky = if (aSignificand << @as(S, @intCast(typeWidth - shift)) != 0) @as(Z, 1) else 0;
        aSignificand = aSignificand >> @as(S, @intCast(shift | sticky));
        aExponent = 0;
    }
