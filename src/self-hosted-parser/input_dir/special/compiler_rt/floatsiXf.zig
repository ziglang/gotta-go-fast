...
fn floatsiXf(comptime T: type, a: i32) T {
    ...
    if (exp <= significandBits) {
        // No rounding needed
        const shift = @as(S, @intCast(significandBits - exp));
        mantissa = @as(Z, @intCast(@as(u32, @bitCast(abs_a)))) << shift ^ implicitBit;
    } else {
        const shift = @as(S, @intCast(exp - significandBits));
        // Round to the nearest number after truncation
        mantissa = @as(Z, @intCast(@as(u32, @bitCast(abs_a)))) >> shift ^ implicitBit;
        // Align to the left and check if the truncated part is halfway over
        const round = @as(u32, @bitCast(abs_a)) << @as(u5, @intCast(31 - shift));
        mantissa += @intFromBool(round > 0x80000000);
        // Tie to even
        mantissa += mantissa & 1;
    }
...
test "floatsisf" {
    ...
    try test_one_floatsisf(@as(i32, @bitCast(@as(u32, @intCast(0x80000000)))), 0xcf000000);
}
...
test "floatsitf" {
    ...
    try test_one_floatsitf(@as(i32, @bitCast(@as(u32, @intCast(0xffffffff)))), 0xbfff0000000000000000000000000000);
    try test_one_floatsitf(@as(i32, @bitCast(@as(u32, @intCast(0x80000000)))), 0xc01e0000000000000000000000000000);
}
