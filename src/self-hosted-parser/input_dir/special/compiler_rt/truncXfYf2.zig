...
pub fn __truncsfhf2(a: f32) callconv(.C) u16 {
    return @as(u16, @bitCast(@call(.{ .modifier = .always_inline }, truncXfYf2, .{ f16, f32, a })));
}

pub fn __truncdfhf2(a: f64) callconv(.C) u16 {
    return @as(u16, @bitCast(@call(.{ .modifier = .always_inline }, truncXfYf2, .{ f16, f64, a })));
}

pub fn __trunctfhf2(a: f128) callconv(.C) u16 {
    return @as(u16, @bitCast(@call(.{ .modifier = .always_inline }, truncXfYf2, .{ f16, f128, a })));
}
...
fn truncXfYf2(comptime dst_t: type, comptime src_t: type, a: src_t) dst_t {
    ...
    const result: dst_rep_t align(@alignOf(dst_t)) = absResult | @as(dst_rep_t, @truncate(sign >> @as(SrcShift, @intCast(srcBits - dstBits))));
