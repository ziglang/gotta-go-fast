...
fn mulXf3(comptime T: type, a: T, b: T) T {
    ...
    var productExponent: i32 = @as(i32, @bitCast(aExponent +% bExponent)) -% exponentBias +% scale;
    ...
    if (productExponent <= 0) {
        ...
        if (shift >= typeWidth) return @as(T, @bitCast(productSign));
        ...
        productHi |= @as(Z, @as(u32, @bitCast(productExponent))) << significandBits;
    }
...
fn wideRightShiftWithSticky(comptime Z: type, hi: *Z, lo: *Z, count: u32) void {
    ...
    if (count < typeWidth) {
        const sticky = @intFromBool((lo.* << @as(S, @intCast(typeWidth -% count))) != 0);
        lo.* = (hi.* << @as(S, @intCast(typeWidth -% count))) | (lo.* >> @as(S, @intCast(count))) | sticky;
        hi.* = hi.* >> @as(S, @intCast(count));
    } else if (count < 2 * typeWidth) {
        const sticky = @intFromBool((hi.* << @as(S, @intCast(2 * typeWidth -% count)) | lo.*) != 0);
        lo.* = hi.* >> @as(S, @intCast(count -% typeWidth)) | sticky;
        hi.* = 0;
    } else {
        const sticky = @intFromBool((hi.* | lo.*) != 0);
        lo.* = sticky;
        hi.* = 0;
    }
}
