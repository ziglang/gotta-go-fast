...
pub fn fixint(comptime fp_t: type, comptime fixint_t: type, a: fp_t) fixint_t {
    ...
    if (@as(usize, @intCast(exponent)) >= fixint_bits) {
    ...
    if (exponent < significandBits) {
        uint_result = @as(UintResultType, @intCast(significand)) >> @as(Log2Int(UintResultType), @intCast(significandBits - exponent));
    } else {
        uint_result = @as(UintResultType, @intCast(significand)) << @as(Log2Int(UintResultType), @intCast(exponent - significandBits));
    }

    // Cast to final signed result
    if (negative) {
        return if (uint_result >= -math.minInt(fixint_t)) math.minInt(fixint_t) else -@as(fixint_t, @intCast(uint_result));
    } else {
        return if (uint_result >= math.maxInt(fixint_t)) math.maxInt(fixint_t) else @as(fixint_t, @intCast(uint_result));
    }
}
