...
pub fn ashlXi3(comptime T: type, a: T, b: i32) T {
    ...
    if (b >= dwords.bits) {
        output.s.low = 0;
        output.s.high = input.s.low << @as(S, @intCast(b - dwords.bits));
    } else if (b == 0) {
        return a;
    } else {
        output.s.low = input.s.low << @as(S, @intCast(b));
        output.s.high = input.s.high << @as(S, @intCast(b));
        output.s.high |= input.s.low >> @as(S, @intCast(dwords.bits - b));
    }
...
pub fn ashrXi3(comptime T: type, a: T, b: i32) T {
    ...
    if (b >= dwords.bits) {
        ...
        output.s.low = input.s.high >> @as(S, @intCast(b - dwords.bits));
    } else if (b == 0) {
        return a;
    } else {
        output.s.high = input.s.high >> @as(S, @intCast(b));
        output.s.low = input.s.high << @as(S, @intCast(dwords.bits - b));
        // Avoid sign-extension here
        output.s.low |= @as(
            dwords.HalfT,
            @bitCast(@as(dwords.HalfTU, @bitCast(input.s.low)) >> @as(S, @intCast(b))),
        );
    }
