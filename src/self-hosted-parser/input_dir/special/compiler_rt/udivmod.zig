...
pub fn udivmod(comptime DoubleInt: type, a: DoubleInt, b: DoubleInt, maybe_rem: ?*DoubleInt) DoubleInt {
    ...
    const n = @as(*const [2]SingleInt, @ptrCast(&a)).*; // TODO issue #421
    const d = @as(*const [2]SingleInt, @ptrCast(&b)).*; // TODO issue #421
    ...
    if (d[low] == 0) {
        ...
        if (n[low] == 0) {
            ...
            if (maybe_rem) |rem| {
                ...
                r[low] = 0;
                rem.* = @as(*align(@alignOf(SingleInt)) DoubleInt, @ptrCast(&r[0])).*; // TODO issue #421
            }
        ...
        if (d[high] == 0) {
            ...
            if ((d[low] & (d[low] - 1)) == 0) {
                ...
                return @as(*align(@alignOf(SingleInt)) DoubleInt, @ptrCast(&q[0])).*; // TODO issue #421
            }
            // K X
            // ---
            // 0 K
    ...
    while (sr > 0) : (sr -= 1) {
        ...
        //      carry = 1;
        // }
        r_all = @as(*align(@alignOf(SingleInt)) DoubleInt, @ptrCast(&r[0])).*; // TODO issue #421
        ...
        r = @as(*[2]SingleInt, @ptrCast(&r_all)).*; // TODO issue #421
    }
    const q_all = ((@as(*align(@alignOf(SingleInt)) DoubleInt, @ptrCast(&q[0])).*) << 1) | carry; // TODO issue #421
    if (maybe_rem) |rem| {
        rem.* = r_all;
    }
    return q_all;
