    ...
    if (sd > FLT_MANT_DIG) {
        ...
        switch (sd) {
            ...
            FLT_MANT_DIG + 2 => {},
                ...
                const shift_amt = @as(u6, @intCast(((N + FLT_MANT_DIG + 2) - sd)));
                const all_ones: u64 = maxInt(u64);
                a = (a >> @as(u6, @intCast(sd - (FLT_MANT_DIG + 2)))) |
                    @intFromBool(a & (all_ones >> shift_amt) != 0);
            },
        }
        // Or P into R
        a |= @intFromBool((a & 4) != 0);
        ...
        a <<= @as(u6, @intCast(FLT_MANT_DIG - sd));
        // a is now rounded to FLT_MANT_DIG bits
    }

    const result: u32 = ((e + 127) << 23) | // exponent
        @as(u32, @truncate(a & 0x007FFFFF)); // mantissa
