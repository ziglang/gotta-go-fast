...
pub const Pdb = struct {
    ...
    pub fn getSymbolName(self: *Pdb, module: *Module, address: u64) ?[]const u8 {
        ...
        while (symbol_i != module.symbols.len) {
            const prefix = @as(*RecordPrefix, @ptrCast(&module.symbols[symbol_i]));
            if (prefix.RecordLen < 2)
                return null;
            switch (prefix.RecordKind) {
                .S_LPROC32, .S_GPROC32 => {
                    const proc_sym = @as(*ProcSym, @ptrCast(&module.symbols[symbol_i + @sizeOf(RecordPrefix)]));
                    if (address >= proc_sym.CodeOffset and address < proc_sym.CodeOffset + proc_sym.CodeSize) {
                        return mem.spanZ(@as([*:0]u8, @ptrCast(proc_sym)) + @sizeOf(ProcSym));
                    }
                },
                else => {},
            }
    ...
    pub fn getLineNumberInfo(self: *Pdb, module: *Module, address: u64) !debug.LineInfo {
        ...
        while (sect_offset != subsect_info.len) : (sect_offset += skip_len) {
            const subsect_hdr = @as(*DebugSubsectionHeader, @ptrCast(&subsect_info[sect_offset]));
            ...
            switch (subsect_hdr.Kind) {
                .Lines => {
                    ...
                    const line_hdr = @as(*LineFragmentHeader, @ptrCast(&subsect_info[line_index]));
                    ...
                    if (address >= frag_vaddr_start and address < frag_vaddr_end) {
                        ...
                        while (line_index < subsection_end_index) {
                            const block_hdr = @as(*LineBlockFragmentHeader, @ptrCast(&subsect_info[line_index]));
                            ...
                            while (line_i < block_hdr.NumLines) : (line_i += 1) {
                                const line_num_entry = @as(*LineNumberEntry, @ptrCast(&subsect_info[line_index]));
                            ...
                            if (line_i > 0) {
                                ...
                                const chksum_hdr = @as(*FileChecksumEntryHeader, @ptrCast(&module.subsect_info[subsect_index]));
                                ...
                                const column = if (has_column) blk: {
                                    ...
                                    const col_num_entry = @as(*ColumnNumberEntry, @ptrCast(&subsect_info[col_index]));
                                    break :blk col_num_entry.StartColumn;
                                } else 0;

                                const found_line_index = start_line_index + line_entry_idx * @sizeOf(LineNumberEntry);
                                const line_num_entry = @as(*LineNumberEntry, @ptrCast(&subsect_info[found_line_index]));
    ...
    pub fn getModule(self: *Pdb, index: usize) !?*Module {
        ...
        while (sect_offset != mod.subsect_info.len) : (sect_offset += skip_len) {
            const subsect_hdr = @as(*DebugSubsectionHeader, @ptrCast(&mod.subsect_info[sect_offset]));
            skip_len = subsect_hdr.Length;
            sect_offset += @sizeOf(DebugSubsectionHeader);
