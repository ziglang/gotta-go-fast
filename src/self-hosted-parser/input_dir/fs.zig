...
pub const Dir = struct {
        ...
        .macos, .ios, .freebsd, .netbsd, .dragonfly, .openbsd, .solaris => struct {
            ...
            fn nextSolaris(self: *Self) !?Entry {
                start_over: while (true) {
                    ...
                    const name = mem.spanZ(@as([*:0]u8, @ptrCast(&entry.d_name)));
        ...
        .haiku => struct {
            ...
            pub fn next(self: *Self) Error!?Entry {
                start_over: while (true) {
                    ...
                    self.index = next_index;
                    const name = mem.spanZ(@as([*:0]u8, @ptrCast(&haiku_entry.d_name)));
        ...
        .linux => struct {
            ...
            pub fn next(self: *Self) Error!?Entry {
                start_over: while (true) {
                    ...
                    const name = mem.spanZ(@as([*:0]u8, @ptrCast(&linux_entry.d_name)));
    ...
    fn openDirAccessMaskW(self: Dir, sub_path_w: [*:0]const u16, access_mask: u32, no_follow: bool) OpenError!Dir {
        ...
        const path_len_bytes = @as(u16, @intCast(mem.lenZ(sub_path_w) * 2));
