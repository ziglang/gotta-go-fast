...
pub const StringIndexContext = struct {
    ...
    pub fn hash(self: @This(), x: u32) u64 {
        const x_slice = mem.spanZ(@as([*:0]const u8, @ptrCast(self.bytes.items.ptr)) + x);
...
pub const StringIndexAdapter = struct {
    ...
    pub fn eql(self: @This(), a_slice: []const u8, b: u32) bool {
        const b_slice = mem.spanZ(@as([*:0]const u8, @ptrCast(self.bytes.items.ptr)) + b);
...
pub fn HashMapUnmanaged(
    ...
    return struct {
        ...
        fn header(self: *const Self) *Header {
            return @as(*Header, @ptrCast(@as([*]Header, @ptrCast(self.metadata.?)) - 1));
        }
