...
pub const f128_true_min = @as(f128, @bitCast(@as(u128, 0x00000000000000000000000000000001)));
pub const f128_min = @as(f128, @bitCast(@as(u128, 0x00010000000000000000000000000000)));
pub const f128_max = @as(f128, @bitCast(@as(u128, 0x7FFEFFFFFFFFFFFFFFFFFFFFFFFFFFFF)));
pub const f128_epsilon = @as(f128, @bitCast(@as(u128, 0x3F8F0000000000000000000000000000)));
...
pub const inf_f16 = @as(f16, @bitCast(inf_u16));
...
pub const inf_f32 = @as(f32, @bitCast(inf_u32));
...
pub const inf_f64 = @as(f64, @bitCast(inf_u64));
...
pub const inf_f128 = @as(f128, @bitCast(inf_u128));
...
pub fn doNotOptimizeAway(value: anytype) void {
    ...
    var x: T = undefined;
    const p = @as(*volatile T, @ptrCast(&x));
    p.* = x;
}
...
pub fn min(x: anytype, y: anytype) Min(@TypeOf(x), @TypeOf(y)) {
    ...
    if (x < y) {
        ...
        switch (@typeInfo(Result)) {
            .Int => return @as(Result, @intCast(x)),
            else => return x,
        }
    } else {
        ...
        // return type.
        switch (@typeInfo(Result)) {
            .Int => return @as(Result, @intCast(y)),
            else => return y,
        }
    }
}
...
test "math.f64_min" {
    ...
    try testing.expect(@as(u64, @bitCast(fmin)) == f64_min_u64);
}
