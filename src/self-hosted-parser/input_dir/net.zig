    ...
    pub fn getOsSockLen(self: Address) os.socklen_t {
        switch (self.any.family) {
            ...
            os.AF.UNIX => {
                ...
                return @as(os.socklen_t, @intCast(@sizeOf(os.sockaddr.un) - self.un.path.len + path_len));
            },
            else => unreachable,
        }
    }
...
pub fn getAddressList(allocator: *mem.Allocator, name: []const u8, port: u16) !*AddressList {
    ...
    if (builtin.target.os.tag == .windows or builtin.link_libc) {
        ...
        if (builtin.target.os.tag == .windows) switch (@as(os.windows.ws2_32.WinsockError, @enumFromInt(@as(u16, @intCast(rc))))) {
            @as(os.windows.ws2_32.WinsockError, @enumFromInt(0)) => {},
...
fn linuxLookupName(
    ...
    for (addrs.items, 0..) |*addr, i| {
        ...
        if (addr.addr.any.family == os.AF.INET6) {
            ...
            da = @as(*os.sockaddr, @ptrCast(&da6));
            dalen = @sizeOf(os.sockaddr.in6);
            sa = @as(*os.sockaddr, @ptrCast(&sa6));
            ...
            da = @as(*os.sockaddr, @ptrCast(&da4));
            dalen = @sizeOf(os.sockaddr.in);
            sa = @as(*os.sockaddr, @ptrCast(&sa4));
