...
fn rawCAlloc(
    ...
    const ptr = @as([*]u8, @ptrCast(c.malloc(len) orelse return error.OutOfMemory));
    return ptr[0..len];
}
...
const PageAllocator = struct {
    fn alloc(allocator: *Allocator, n: usize, alignment: u29, len_align: u29, ra: usize) error{OutOfMemory}![]u8 {
        ...
        if (builtin.os.tag == .windows) {
            ...
            if (mem.isAligned(@intFromPtr(addr), alignment)) {
                return @as([*]u8, @ptrCast(addr))[0..alignPageAllocLen(aligned_len, n, len_align)];
            }
            ...
            while (true) {
                ...
                const aligned_addr = mem.alignForward(@intFromPtr(reserved_addr), alignment);
                ...
                const ptr = w.VirtualAlloc(
                    @as(*anyopaque, @ptrFromInt(aligned_addr)),
                    aligned_len,
                    w.MEM_COMMIT | w.MEM_RESERVE,
                    w.PAGE_READWRITE,
                ) catch continue;

                return @as([*]u8, @ptrCast(ptr))[0..alignPageAllocLen(aligned_len, n, len_align)];
            }
        }
        ...
        const new_hint: [*]u8 = @alignCast(result_ptr + aligned_len);
    ...
    fn resize(
        ...
        if (new_size_aligned < buf_aligned_len) {
            const ptr: [*]u8 = @alignCast(buf_unaligned.ptr + new_size_aligned);
...
const WasmPageAllocator = struct {
    ...
    fn alloc(allocator: *Allocator, len: usize, alignment: u29, len_align: u29, ra: usize) error{OutOfMemory}![]u8 {
        ...
        return @as([*]u8, @ptrFromInt(page_idx * mem.page_size))[0..alignPageAllocLen(page_count * mem.page_size, len, len_align)];
    }
...
pub const HeapAllocator = switch (builtin.os.tag) {
    .windows => struct {
        ...
        fn alloc(
            ...
            const buf = @as([*]u8, @ptrFromInt(aligned_addr))[0..return_len];
...
test "WasmPageAllocator internals" {
    if (comptime builtin.target.isWasm()) {
        ...
        try testing.expect(@intFromPtr(extended.ptr) >= conventional_memsize);
...
pub fn testAllocatorAlignedShrink(base_allocator: *mem.Allocator) !void {
    ...
    while (@intFromPtr(slice.ptr) == mem.alignForward(@intFromPtr(slice.ptr), mem.page_size * 32)) {
