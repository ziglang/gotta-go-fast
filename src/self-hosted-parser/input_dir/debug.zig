...
pub fn captureStackTrace(first_address: ?usize, stack_trace: *std.builtin.StackTrace) void {
    if (native_os == .windows) {
        ...
        const u32_addrs_len = @as(u32, @intCast(addrs.len));
        const first_addr = first_address orelse {
            stack_trace.index = windows.ntdll.RtlCaptureStackBackTrace(
                0,
                u32_addrs_len,
                @as(**anyopaque, @ptrCast(addrs.ptr)),
                null,
            );
            return;
        };
        ...
        const n = windows.ntdll.RtlCaptureStackBackTrace(0, u32_addrs_len, @as(**anyopaque, @ptrCast(addr_buf.ptr)), null);
...
pub fn writeCurrentStackTraceWindows(
    ...
    const n = windows.ntdll.RtlCaptureStackBackTrace(0, addr_buf.len, @as(**anyopaque, @ptrCast(&addr_buf)), null);
...
fn readMachODebugInfo(allocator: *mem.Allocator, macho_file: File) !ModuleDebugInfo {
    ...
    const hdr_base = @as([*]const u8, @ptrCast(hdr));
    var ptr = hdr_base + @sizeOf(macho.mach_header_64);
    var ncmd: u32 = hdr.ncmds;
    const symtab = while (ncmd != 0) : (ncmd -= 1) {
        const lc = @as(*const std.macho.load_command, @ptrCast(ptr));
        switch (lc.cmd) {
            std.macho.LC_SYMTAB => break @as(*const std.macho.symtab_command, @ptrCast(ptr)),
            else => {},
        }
        ptr = @alignCast(ptr + lc.cmdsize);
    } else {
        return error.MissingDebugInfo;
    };
    const syms = @as([*]const macho.nlist_64, @ptrCast(@alignCast(hdr_base + symtab.symoff)))[0..symtab.nsyms];
    const strings = @as([*]const u8, @ptrCast(hdr_base + symtab.stroff))[0 .. symtab.strsize - 1 :0];
...
pub const DebugInfo = struct {
    ...
    fn lookupModuleDyld(self: *DebugInfo, address: usize) !*ModuleDebugInfo {
        ...
        while (i < image_count) : (i += 1) {
            ...
            var cmd_ptr = @as([*]u8, @ptrFromInt(@intFromPtr(header) + @sizeOf(macho.mach_header_64)));
            ...
            while (cmds != 0) : (cmds -= 1) {
                ...
                const segment_cmd = @as(
                    *const std.macho.segment_command_64,
                    @ptrCast(@alignCast(lc)),
                );
    ...
    fn lookupModuleWin32(self: *DebugInfo, address: usize) !*ModuleDebugInfo {
        ...
        if (windows.kernel32.K32EnumProcessModules(
            process_handle,
            @as([*]windows.HMODULE, @ptrCast(&dummy)),
            0,
            &bytes_needed,
        ) == 0)
            return error.MissingDebugInfo;
        ...
        for (modules[0..loaded_modules]) |module| {
            ...
            const seg_start = @intFromPtr(info.lpBaseOfDll);
            ...
            if (address >= seg_start and address < seg_end) {
                ...
                const len = windows.kernel32.K32GetModuleFileNameExW(
                    process_handle,
                    module,
                    @as(windows.LPWSTR, @ptrCast(&name_buffer[4])),
    ...
    .macos, .ios, .watchos, .tvos => struct {
        ...
        fn loadOFile(self: *@This(), o_file_path: []const u8) !DW.DwarfInfo {
            ...
            const sections = @as(
                ...
                @ptrCast(@alignCast(ptr + @sizeOf(std.macho.segment_command_64))),
            )[0..segcmd.nsects];
            for (sections) |*sect| {
...
fn handleSegfaultLinux(sig: i32, info: *const os.siginfo_t, ctx_ptr: ?*const anyopaque) callconv(.C) noreturn {
    ...
    switch (native_arch) {
        .i386 => {
            const ctx = @as(*const os.ucontext_t, @ptrCast(@alignCast(ctx_ptr)));
        ...
        .aarch64 => {
            const ctx = @as(*const os.ucontext_t, @ptrCast(@alignCast(ctx_ptr)));
            const ip = @as(usize, @intCast(ctx.mcontext.pc));
            // x29 is the ABI-designated frame pointer
            const bp = @as(usize, @intCast(ctx.mcontext.regs[29]));
            dumpStackTraceFromBase(bp, ip);
        },
        else => {},
    }
