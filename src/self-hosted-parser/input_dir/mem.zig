...
pub fn ValidationAllocator(comptime T: type) type {
    return struct {
        ...
        pub fn alloc(
            ...
            assert(mem.isAligned(@intFromPtr(result.ptr), ptr_align));
            if (len_align == 0) {
                assert(result.len == n);
            } else {
                assert(result.len >= n);
            ...
            switch (@typeInfo(Init)) {
                .Struct => |init_info| {
                    ...
                    if (init_info.is_tuple) {
                        inline for (init_info.fields, 0..) |field, i| {
...
pub fn alignPointerOffset(ptr: anytype, align_to: u29) ?usize {
    assert(align_to > 0 and std.math.isPowerOfTwo(align_to));
...
pub fn asBytes(ptr: anytype) AsBytesReturnType(@TypeOf(ptr)) {
    const P = @TypeOf(ptr);
    return @as(AsBytesReturnType(P), @ptrCast(ptr));
}
...
pub fn isValidAlign(alignment: u29) bool {
    return std.math.isPowerOfTwo(alignment);
}
